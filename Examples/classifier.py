...
import tqdm
from monotonenorm import direct_norm, MonotonicWrapper, GroupSort
...
if __name__ == "__main__":
    ...
    for i in (pbar := tqdm.trange(EPOCHS)):
        model.train()
        optimizer.zero_grad()
        y_pred = model(X)
    ...
    x0mesh, x1mesh = torch.meshgrid(
        torch.linspace(*range_x0, 20),
        torch.linspace(*range_x1, 20),
        indexing="ij",
    )
